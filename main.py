...
def main(argv=None) -> int:
    ...
    ap.add_argument("--threshold-percentile", type=float, default=82.0, help="自适应阈值百分位")
    ap.add_argument("--threshold-floor", type=float, default=0.3, help="阈值地板(绝对可见阈值, 融合图量级)")
    ...
    if args.image:
        ...
        print(f"[1/6] 本地原图: {original.shape[1]}x{original.shape[0]}")
    else:
        ...
        if p is None:
            ...
            return 1
        original = load_rgb(p)
        print(f"[1/6] 原图: {original.shape[1]}x{original.shape[0]}")

    # 2. 压缩
    compressed = load_rgb(compress_jpeg(original, out / "compressed.jpg",
                                        quality=args.jpeg_quality))
    print(f"[2/6] JPEG q{args.jpeg_quality} 压缩完成 (4:2:0)")
    ...
    print("[5/6] 差异评价完成 (errormap + mask + overlay)")
